...
for sheet in bk.sheets():   #读每一个表格
    ...
    for i in range(1,20):   #i=1----19
        ...
        unit[i] = int(float(0.01 * Account / N[i])/100)*100  #交易单位（股）
    for i in range(20,numOfSamples): #i=20----266
        ...
        unit[i] = int(float(0.01 * Account / N[i])/100)*100
    ...
    df1 = pd.DataFrame(np.column_stack((N[20:],unit[20:],unit_all[20:],cost[20:],da[20:,0],trade_flag[20:],delta_unit[20:])),columns=['N','unit','unit_all','cost','date','trade','delta'])
